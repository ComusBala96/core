...
export function addCustomFonts(pdfMake, fonts = []) {
...
/* ---------------------------
   Add Table Layouts
----------------------------*/
export function addCustomTableLayouts(pdfMake, layouts = []) {
...
export function htmlToPdfMake(op) {
...
export function createDomObject(element) {
    ...
    if (pdfAttr)
        obj.pdf = Obj.jsonParse(pdfAttr);
    const styleAttr = element.getAttribute('data-style');
    if (styleAttr)
        obj = { ...obj, ...Obj.jsonParse(styleAttr) };
...
export function getQrCodeObject(element) {
...
export function getInlineText(element) {
    const styles = Obj.jsonParse(element.getAttribute('data-style') || '{}');
...
export function getSvgObject(element) {
...
export function getImageObject(element) {
    return {
        image: element.getAttribute('alt') || 'no_name_given',
    };
}
export function getTableObject(element) {
...
export function mapChildren(element, type) {
    const stack = [];
...
export function cssToCamelCase(prop) {
    return prop.replace(/-([a-z])/g, (_, l) => l.toUpperCase());
}
export function tableStack(rows, op, images) {
