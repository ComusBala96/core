...
export function getUpdatedTableData(element, api, updateCols) {
    const data = {};
    if (!api)
        return Obj.getDataAttr(element);
...
export function getSelectedTableData(element, selected, tableLoadType = 'ajax') {
    const ids = selected
        .map((row) => {
