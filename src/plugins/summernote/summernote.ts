...
function uploadImage(file: File, element: string) {
    if (Config.app_env) console.log('Uploading', file);

    const formData = new FormData();
    if (Config.csrf_token && Config.locale) {
        formData.append('file', file);
        formData.append('_token', Config.csrf_token);
        formData.append('lang', Config.locale);
    }

    $.ajax({
        url: `${Config.app_url}upload/summernote`,
        method: 'POST',
        data: formData,
        cache: false,
        processData: false,
        ...
        success(res: string) {
            if (Config.app_env) console.log('summernote response', res);
            if (handleUploadError(res)) return;
            insertImage(res, element);
        },
        error(xhr: JQuery.jqXHR<any>) {
            if (Config.app_env) console.error(xhr.responseText);
        },
    });
}
function insertImage(filename: string, element: string) {
    ...
    img.src = `${Config.app_url}${Config.paths.summernote}${filename}`;
    img.alt = filename;
...
function deleteMedia(imgUrl: string) {
    ...
    $.ajax({
        url: `${Config.app_url}delete/summernote`,
        method: 'POST',
        data: {
            img: file,
