...
const DEFAULT_RESOURCE_ACTIONS: RouteAction[] = ['index', 'create', 'edit', 'save', 'delete'];

export function custom(patterns: string | string[], importer: PageImporter, meta?: RouteMeta): RouteDefinition {
    return {
...
export function group(prefix: string, routes: RouteDefinition[], options?: GroupOptions): RouteDefinition[] {
...
export function resource(base: string, importer: PageImporter, options: ResourceOptions = {}): RouteDefinition {
...
export function only(actions: RouteAction[], importer: PageImporter, base: string, options?: ResourceOptions): RouteDefinition {
...
export function except(actions: RouteAction[], importer: PageImporter, base: string, options?: ResourceOptions): RouteDefinition {
    return resource(base, importer, {
        ...options,
        except: actions,
    });
...
export function shiftParams(pattern: string): string {
    const parts = Str.split(pattern);

    if (parts.length < 3) return Str.normalize(pattern);
...
export function unshiftParams(pattern: string): string {
    const parts = Str.split(pattern);
